...
describe('readConfiguration', () => {
  [undefined, {}, [], 0].forEach(nonStringFilePath => {
    it(`should exit with error message if given configuration file path is not a string ${JSON.stringify(
      nonStringFilePath,
    )}`, () => {
      expect(() => readConfiguration(nonStringFilePath)).to.throw(
        TypeError,
  ...
  it('should exit with error message if given configuration file does not exist', () => {
    const readConfigurationFromNonExistingFile = () =>
      readConfiguration('some/file/that/does/not/exist.js');
    expect(readConfigurationFromNonExistingFile).to.throw(
      TypeError,
      path.join('some/file/that/does/not/exist.js'),
    );
  });
  ...
  context('if given a configuration file', () => {
    ...
    it('should import and return it', () => {
      expect(
        readConfiguration('test/fixtures/configuration-reader.test/valid.config.js').environments,
      ).to.deep.equal([
        {
          extension: 'js',
          testNameSuffix: '.test',
    ...
    it('should lowercase each environment extension automatically', () => {
      const config = readConfiguration(
        'test/fixtures/configuration-reader.test/technically.valid.config.js',
      );
    ...
    it('should assign empty array to arguments property for testRunnerCommand if missing', () => {
      const config = readConfiguration(
        'test/fixtures/configuration-reader.test/no-test-command-arguments.config.js',
      );
